...
function enqueueLink(link, instance)
{
	...
	try{
		...
		let protocol = '';
		if (typeof link.url.original == "string" && link.url.original.search(':') >= 0){
			protocol = link.url.original.split(':')[0]
		}
		link.url.parsed = {
			protocol: protocol+':',
			slashes: null,
			...
			extra: {
				protocolTruncated: protocol,
				portIsDefault: true,
				directory: [],
				directoryLeadingSlash: true,
				filename: null,
				filenameIsIndex: true,
				query: {},
				type: 0
			}
		}
		link.url.resolved = link.url.original
		link.unknown = true
		link.resolved = true
	}
